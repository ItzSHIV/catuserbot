...
def file_check(re=True, me=True, mo=True, it=True, fa=True, sp=True, go=False):
    ...
    if sp and not os.path.exists(special):
        urllib.request.urlretrieve(
            "hhttps://github.com/ItzSHIV/Userbot-Resources/blob/master/Resources/Spotify/ArialUnicodeMS.ttf?raw=true",
            special,
        )
    if go and not os.path.exists(google):
        urllib.request.urlretrieve(
            "https://github.com/ItzSHIV/Userbot-Resources/blob/master/Resources/Spotify/GoogleSans-Bold.ttf?raw=true",
            google,
        )
